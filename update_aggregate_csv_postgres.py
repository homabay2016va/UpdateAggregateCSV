...
data = pd.read_csv("newdeaths.csv")

##USAGE
filename="newdeaths.csv"
formid='preloadv1'
UpdateData(filename,formid)
...
def UpdateData(filename,formid):
    try:
        connection = p.connect(user="dbuser",
                               password="dbpass",
                               host="127.0.0.1",
                               port="5432",
                               database="dbname")
        ...
        #print("you are connected ",record,"\n")
        uid = uuid.uuid1()
        uid= "uuid:"+str(uid)
        ...
        sql_update_blob_query = """ update _form_info_manifest_blb t1
    set "VALUE"= %s, "_URI"=%s, "_LAST_UPDATE_DATE"=%s,"_CREATION_DATE"=%s
    from _form_info_manifest_ref t2 
      inner join _form_info_manifest_bin t3 on 
    t2."_DOM_AURI" = t3."_URI" where t3."UNROOTED_FILE_PATH"='"""+filename+"""' and t1."_URI" = t2."_SUB_AURI" """

        sql_update_blob_query2 = """ update _form_info_manifest_bin t1
        set "CONTENT_LENGTH"= %s, "CONTENT_HASH"= %s ,"_LAST_UPDATE_DATE"=%s
        from _form_info t2 where
        t1."_TOP_LEVEL_AURI"=t2."_URI" and t2."FORM_ID"='"""+formid+"""' and t1."UNROOTED_FILE_PATH"='"""+filename+"""' """

        sql_update_blob_query3 = """ update _form_info_manifest_ref t1
         set "_SUB_AURI"= %s, "_LAST_UPDATE_DATE"=%s, "_CREATION_DATE"=%s
        from  _form_info_manifest_bin t2 where
        t1."_DOM_AURI" = t2."_URI" and t2."UNROOTED_FILE_PATH"='"""+filename+"""' """

        sql_update_blob_query4 = """ update _form_info_fileset t1 set "_LAST_UPDATE_DATE"=%s
        from _form_info t2 
        where t1."_PARENT_AURI"=t2."_URI" and t2."FORM_ID"='"""+formid+"""' """

        sql_update_blob_query5 = """ update _form_info set "_LAST_UPDATE_DATE"=%s where "FORM_ID"='"""+formid+"""' """
